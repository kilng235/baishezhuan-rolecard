export const Schema = z.object({
  ...
  白素贞: z.object({
    ...
    服饰: z.object({
      ...
      贴身层: z.string().prefault('藕荷绸肚兜藕带系于颈后,裙内不着裈,以裳蔽体,尽掩于层层裙衫之下,不见分毫'),
      下装: z.string().prefault('浅紫百褶裙腰头束紧,腰身盈盈一握,裙摆绣白兰,边缘微湿'),
      鞋袜: z.string().prefault('藕荷罗袜齐踝,浅紫绣鞋绣紫莲,鞋面浅了一点雨星,尚不算湿透'),
  ...
  小青: z.object({
    ...
    服饰: z.object({
      ...
      贴身层: z.string().prefault('鹅黄绸肚兜,裙内不着裈,以裳蔽体,被外衫罗裙层层掩住,不见分毫'),
      下装: z.string().prefault('鹅黄细葛裙裙摆垂落,边缘浅沾雨星,衣料轻盈不显湿重'),
      鞋袜: z.string().prefault('绣鞋鹅黄面绣黄蝶,鞋面微湿。脚踝处用罗袜掩了大半那圈淡淡的青鳞纹,只余一线若隐若现'),
