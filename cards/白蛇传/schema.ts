export const Schema = z.object({
  系统变量: z.object({
    日期: z.string().prefault('1147-04-05'),
    时间段: z.enum(['清晨', '上午', '午后', '黄昏', '夜晚']).prefault('午后'),
  }).prefault({}),
  白素贞: z.object({
    当前位置: z.string().prefault('西湖断桥'),
    好感度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(60),
    外貌: z.string().prefault('眉目清丽如画，唇色浅淡不施粉黛，素净得像水墨里走出来的人。眼底青碧色的蛇瞳敛在幽处，此刻正看着亭下那人。鬓边几缕碎发被细雨沾湿，贴在耳畔'),
    服饰: z.string().prefault('月白软烟罗中衣领口滚一道银线,襟口微敞,锁骨若隐若现,胸前衣料撑起圆润弧度,衣襟处沾了几点细密的雨珠。外罩藕荷刻丝银线褙子,直领对襟,开衩至腰下,起花八团,腰身收得极窄,袖口和裙摆下沿微微洇了一片水色。藕荷绸肚兜绣一枝紫莲,贴身勾勒丰盈曲线。浅紫百褶裙腰头束紧,腰身盈盈一握,裙摆绣白兰,边缘微湿。藕荷绣鞋绣紫莲,鞋面浅了一点雨星,尚不算湿透。云髻高绾,紫玉簪簪头雕莲花,鬓边一枝鹅黄绒花沾了几颗雨珠。腰系浅金宫绦垂一块羊脂玉佩,臂挽浅紫披帛银线云纹,风过时轻轻扬起又落下'),
    姿势: z.string().prefault('立在桥亭亭柱旁,刚刚伸手接过许仙递来的一把青布伞,手指停在伞柄上'),
    心理: z.string().prefault(''),
  }).prefault({}),
  小青: z.object({
    当前位置: z.string().prefault('西湖断桥'),
    好感度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(20),
    外貌: z.string().prefault('一张瓜子脸,眉眼灵动,笑起来颊边两个浅梨涡。瞳色刻意收敛装作寻常乌眼,只眼尾微微泛着一点翠色,像是戴了副薄薄的碧色琉璃目。嘴角微微翘着,方才冲那人喊了一嗓子,面上还带着几分促狭。发梢上挂着几滴细雨'),
    服饰: z.string().prefault('鹅黄蝉翼纱褙子薄如蝉翼,风一过便掀起来,领口敞着未系,衣带松松挽个结,衫子上星星点点沾了几点雨珠。鹅黄细葛衫交领,袖口挽起半截,衣料绷在胸前鼓鼓撑起。鹅黄绸肚兜绣一只黄蝶,随呼吸轻轻起伏。鹅黄亵裤宽腿裤脚卷到膝弯,露一截白皙小腿,微有雨珠滚落。鹅黄细葛裙裙摆挽到膝上,露出白皙小腿,裙角浅沾雨星。绣鞋鹅黄面绣黄蝶,鞋面微湿。发髻松松挽着几缕青丝,鬓边一枝银钗雕成蝴蝶模样,钗头挂着几颗雨珠。腕上一只细银镯沾了点水渍。脚踝处用绣袜、长裙、缠足布层层遮掩了那圈淡淡的青鳞纹。臂搭鹅黄绡披帛一端垂在身侧,被细雨压得稍稍服帖'),
    姿势: z.string().prefault('站在姐姐半步之后,刚喊完许仙借伞的话,正歪着头打量他'),
    心理: z.string().prefault(''),
  }).prefault({}),
});
export type Schema = z.output<typeof Schema>;
